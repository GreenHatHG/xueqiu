...
2. `evidence_refs` 里的每个 `quote` **必须**从对应节点的 `text` 里截取（精确子串匹配，原样复制，标点/表情/空格也必须一致）。严禁从 `quoted_text` 里取 `quote`；`quoted_text` 只能当背景理解。
...
- `evidence_refs` 的每一项也必须指向 speaker 为 __FOCUS_USERNAME__ 的树节点；严禁拿别人发言当证据。
- `evidence_refs` 至少要有 1 项；默认只写 1 项。只有当你能从同一条消息的 `text` 里直接复制出多段原文子串时，才增加到 2+ 项；不要为了凑数引用 `quoted_text` 或改写 `quote`。
